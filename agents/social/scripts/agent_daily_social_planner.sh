...
DEFAULT_SCREENSHOTS_REPO_DIR="$(cd "$AGENTS_REPO_DIR/.." && pwd)/hushline-screenshots"
SCREENSHOTS_REPO_DIR="${HUSHLINE_SCREENSHOTS_REPO_DIR:-$DEFAULT_SCREENSHOTS_REPO_DIR}"
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        cat <<'EOF'
Usage:
  ./agents/social/scripts/agent_daily_social_planner.sh --date 2026-03-19
  ./agents/social/scripts/agent_daily_social_planner.sh --date 2026-03-19 --archive-key 2026-03-19-1
...
build_context() {
  local -a cmd=(node "$AGENTS_REPO_DIR/agents/social/scripts/plan-day.js" --date "$DATE")
...
validate_and_render() {
  local -a cmd=(node "$AGENTS_REPO_DIR/agents/social/scripts/validate-day-plan.js" --date "$DATE")
  local output=""
  local rc=0
  [[ -n "$ARCHIVE_KEY" ]] && cmd+=(--archive-key "$ARCHIVE_KEY")
  [[ -n "$CANDIDATE_COUNT" ]] && cmd+=(--candidate-count "$CANDIDATE_COUNT")
...
push_archive() {
  ...
  local -a cmd=("$AGENTS_REPO_DIR/agents/social/scripts/push_previous_posts_archive.sh" --date "$DATE")
  [[ -n "$ARCHIVE_KEY" ]] && cmd+=(--archive-key "$ARCHIVE_KEY")
  "${cmd[@]}"
}
...
verify_screenshot_source() {
  ...
  if [[ -d "${HUSHLINE_CURRENT_SCREENSHOTS_DIR:-}" ]]; then
    current_fold_count="$(find "$HUSHLINE_CURRENT_SCREENSHOTS_DIR" -type f -name '*-fold.png' | wc -l | tr -d '[:space:]')"
    echo "Current screenshots folder: $HUSHLINE_CURRENT_SCREENSHOTS_DIR fold_screenshots=$current_fold_count"
    if [[ "$current_fold_count" =~ ^[0-9]+$ ]] && (( current_fold_count > 0 )); then
      return
    fi
  fi
  ...
  if [[ "$SCREENSHOT_AUTO_SYNC" == "1" ]]; then
    ...
    node "$AGENTS_REPO_DIR/agents/social/scripts/sync-latest-screenshots.js" --dest "$SCREENSHOTS_REPO_DIR/releases/latest"
