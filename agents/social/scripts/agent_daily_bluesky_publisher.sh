...
bluesky_already_published() {
  ...
  archive_path="$archive_root/$archive_key/bluesky-publication.json"
  remote_ref="refs/remotes/$remote/$branch"

  if [[ -f "$REPO_DIR/$archive_path" ]]; then
    echo "$archive_label container $archive_key for planned date $publish_date already has a local Bluesky publication record; skipping publish."
    exit 0
  fi

  if ! git -C "$REPO_DIR" fetch --quiet "$remote" "$branch:$remote_ref"; then
    echo "Failed to refresh $remote/$branch before checking Bluesky publication state." >&2
    exit 1
  fi

  if git -C "$REPO_DIR" cat-file -e "${remote}/${branch}:${archive_path}" 2>/dev/null; then
    echo "$archive_label container $archive_key for planned date $publish_date already has a Bluesky publication record on $remote/$branch; skipping publish."
    exit 0
  fi
}
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        cat <<'EOF'
Usage:
  ./agents/social/scripts/agent_daily_bluesky_publisher.sh
  ./agents/social/scripts/agent_daily_bluesky_publisher.sh --date 2026-03-18
  ./agents/social/scripts/agent_daily_bluesky_publisher.sh --date 2026-03-18 --archive-key 2026-03-18-1
  ./agents/social/scripts/agent_daily_bluesky_publisher.sh --date 2026-04-01 --date-root previous-article-posts
  ./agents/social/scripts/agent_daily_bluesky_publisher.sh --dry-run

Behavior:
  - Loads the archived social post from the selected archive root
  - Finds the post for today or the supplied date
  - Publishes it to Bluesky
  - Writes bluesky-publication.json
...
skip_if_weekend() {
  ...
  if [[ "$weekday" == "6" || "$weekday" == "7" ]]; then
    echo "Skipping daily Bluesky publisher for weekend date $publish_date."
    exit 0
  fi
}
...
push_archive() {
  ...
  local -a cmd=(
    "$AGENTS_REPO_DIR/agents/social/scripts/push_previous_posts_archive.sh"
    --date "$(effective_date)"
...
main() {
  ...
  local -a cmd=(node "$AGENTS_REPO_DIR/agents/social/scripts/publish-daily-bluesky.js")
