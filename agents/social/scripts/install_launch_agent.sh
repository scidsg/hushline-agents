  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        cat <<'EOF'
Usage:
  ./agents/social/scripts/install_launch_agent.sh
...
main() {
  ...
  "$AGENTS_REPO_DIR/agents/social/scripts/check_launchd_prereqs.sh" \
    --scope "$SCOPE" \
    --env-file "$ENV_FILE" \
    --owner-user "$APP_USER"
  ...
  case "$SCOPE" in
    gui)
      ...
      install_gui_unit \
        "$AGENTS_REPO_DIR/agents/social/deploy/launchd/com.hushline.social.whistleblower-news-post-agent.plist" \
        "$GUI_TARGET_DIR/com.hushline.social.whistleblower-news-post-agent.plist" \
        "com.hushline.social.whistleblower-news-post-agent"
      install_gui_unit \
        "$AGENTS_REPO_DIR/agents/social/deploy/launchd/com.hushline.social.hushline-feature-post-agent.plist" \
        "$GUI_TARGET_DIR/com.hushline.social.hushline-feature-post-agent.plist" \
        "com.hushline.social.hushline-feature-post-agent"
      install_gui_unit \
        "$AGENTS_REPO_DIR/agents/social/deploy/launchd/com.hushline.social.hushline-verified-user-post-agent.plist" \
    ...
    daemon)
      ...
      install_daemon_unit \
        "$AGENTS_REPO_DIR/agents/social/deploy/launchd/com.hushline.social.whistleblower-news-post-agent.daemon.plist" \
        "$SYSTEM_TARGET_DIR/com.hushline.social.whistleblower-news-post-agent.plist" \
        "com.hushline.social.whistleblower-news-post-agent"
      install_daemon_unit \
        "$AGENTS_REPO_DIR/agents/social/deploy/launchd/com.hushline.social.hushline-feature-post-agent.daemon.plist" \
        "$SYSTEM_TARGET_DIR/com.hushline.social.hushline-feature-post-agent.plist" \
        "com.hushline.social.hushline-feature-post-agent"
      install_daemon_unit \
        "$AGENTS_REPO_DIR/agents/social/deploy/launchd/com.hushline.social.hushline-verified-user-post-agent.daemon.plist" \
...
- optional Codex defaults: CODEX_MODEL=gpt-5.6-sol, CODEX_REASONING_EFFORT=high
