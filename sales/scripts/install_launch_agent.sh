  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        cat <<'EOF'
Usage:
  ./sales/scripts/install_launch_agent.sh
...
install_gui_unit() {
  ...
  render_plist \
    "$AGENTS_REPO_DIR/sales/deploy/launchd/com.hushline.sales.contact-agent.plist" \
    "$target_plist"
...
install_daemon_unit() {
  ...
  render_plist \
    "$AGENTS_REPO_DIR/sales/deploy/launchd/com.hushline.sales.contact-agent.daemon.plist" \
    "$target_plist"
...
- dry run: $AGENTS_REPO_DIR/sales/scripts/run_sales_contact_agent_launchd.sh --dry-run
EOF
}
