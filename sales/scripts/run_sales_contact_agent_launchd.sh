  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)
        cat <<'EOF'
Usage:
  ./sales/scripts/run_sales_contact_agent_launchd.sh
  ./sales/scripts/run_sales_contact_agent_launchd.sh --dry-run
  ./sales/scripts/run_sales_contact_agent_launchd.sh --date 2026-06-10
...
run_agent() {
  local -a command=(
    python3
    "$AGENTS_REPO_DIR/sales/scripts/sales_contact_agent.py"
    --docs-repo-dir
    "$DOCS_REPO_DIR"
  )
