...
"""
    ...
    assert Path(result.stdout.strip()) == ROOT.parent / "hushline"
...
"""
    ...
    assert result.stdout.strip() == str(
        ROOT.parent / "hushline" / ".git" / "hushline-code-agent.lock"
    )
        ...
        "\n".join(
            [
                ...
                (
                    ...
                    "[workdir, /tmp, $TMPDIR, /Users/developer/.codex/memories]"
                ),
...
git() {{
  if [[ "$1 $2 $3 $4" == "config --local --get user.signingkey" ]]; then
    printf '%s\\n' "$repo_dir/.ssh/bot-signing.pub"
    return 0
  fi
  if [[ "$1 $2 $3 $4" == "config --local --get gpg.format" ]]; then
    printf 'ssh\\n'
    return 0
  fi
  printf 'unexpected git invocation: %s\\n' "$*" >&2
...
def test_recent_failure_block_from_text_extracts_recent_actionable_context() -> None:
    failure_text = (
        ...
        "$'/Users/developer/hushline/tests/test_module.py:12:34: "
    ...
    shell_script = f"""
source {shlex.quote(str(RUNNER_SCRIPT))}
REPO_DIR=/Users/developer/hushline
{failure_text}
...
"""
    ...
    assert "/Users/developer/hushline" not in result.stdout
    assert "tests/test_module.py:12:34: F821 Undefined name `MissingName`" in result.stdout
    assert "FAILED tests/test_example.py::test_case" in result.stdout
